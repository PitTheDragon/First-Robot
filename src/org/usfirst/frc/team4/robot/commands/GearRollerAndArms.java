package org.usfirst.frc.team4.robot.commands;

import org.usfirst.frc.team4.robot.Robot;
import org.usfirst.frc.team4.robot.ControllerConstants;
import edu.wpi.first.wpilibj.command.Command;

/**
 *
 */
public class GearRollerAndArms extends Command {

	public static boolean turnOff = false;
	
    public GearRollerAndArms() {
        requires(Robot.gearintake);
    }

    // Called just before this Command runs the first time
    protected void initialize() {
    }

    // Called repeatedly when this Command is scheduled to run
    protected void execute() {
    	if (ControllerConstants.operatorRightBumper.get()) {
			Robot.gearintake.setRollerSpeed(.75);
		} else if (ControllerConstants.operatorLeftBumper.get()) {
			Robot.gearintake.setRollerSpeed(-.75);
		} else {
			Robot.gearintake.rollerStop();
		}		
		double value = ControllerConstants.joystickoperator.getRawAxis(ControllerConstants.AXIS_LEFT_Y);
    }
		// Make this return true when this Command no longer needs to run execute()
    protected boolean isFinished() {
        return false;
    }

    // Called once after isFinished returns true
    protected void end() {
    }

    // Called when another command which requires one or more of the same
    // subsystems is scheduled to run
    protected void interrupted() {
    	end();
    }
}
